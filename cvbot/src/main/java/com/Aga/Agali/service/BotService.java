...
public class BotService {
    ...
    public void handleUpdate(Update update) {
        if (update.hasMessage()) {
            ...
            if (message.hasText()) {
                ...
                if ("/stop".equals(text)) { handleStop(user); return; }
    ...
    private void handleCallback(CallbackQuery callback) {
        ...
        try { getBot().execute(new AnswerCallbackQuery(callback.getId())); }
        catch (TelegramApiException e) { log.error("AnswerCallback xətası: {}", e.getMessage()); }

        switch (data) {
            case "template_1" -> handleTemplateSelect(user, CvTemplate.TEMPLATE_1);
            case "template_2" -> handleTemplateSelect(user, CvTemplate.TEMPLATE_2);
            case "template_3" -> handleTemplateSelect(user, CvTemplate.TEMPLATE_3);
            case "template_4" -> handleTemplateSelect(user, CvTemplate.TEMPLATE_4);
            case "template_5" -> handleTemplateSelect(user, CvTemplate.TEMPLATE_5);
            case "exp_yes" -> { sendText(chatId, "Növbəti iş yerinizin adını daxil edin:"); userService.updateState(user, UserState.COLLECTING_EXP_COMPANY); }
            case "exp_no"  -> { sendText(chatId, "İndi təhsilinizi toplayacağıq."); startEduCollection(user); }
            case "edu_bakalavr" -> { tempEduData.computeIfAbsent(chatId, k -> new HashMap<>()).put("degree", "Bakalavr"); sendText(chatId, "Bakalavr universitetinin adını daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_BACHELOR_UNIVERSITY); }
            case "edu_magistr"  -> { tempEduData.computeIfAbsent(chatId, k -> new HashMap<>()).put("nextDegree", "Magistr"); sendText(chatId, "Magistr dərəcəniz üçün əvvəlcə Bakalavr məlumatlarınızı daxil edin.\n\nBakalavr universitetinin adını daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_BACHELOR_UNIVERSITY); }
            case "edu_kollec"   -> { tempEduData.computeIfAbsent(chatId, k -> new HashMap<>()).put("degree", "Kollec"); sendText(chatId, "Kollec adını daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_COLLEGE_UNIVERSITY); }
            case "edu_yoxdur"   -> { sendText(chatId, "Bacarıqlarınızı daxil edin:\n\nVergüllə ayırın:\n(məs: Java, Spring Boot, SQL)"); userService.updateState(user, UserState.COLLECTING_SKILLS); }
            case "edu_more_bakalavr" -> { tempEduData.computeIfAbsent(chatId, k -> new HashMap<>()).put("degree", "Bakalavr"); sendText(chatId, "Universitetin adını daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_BACHELOR_UNIVERSITY); }
            case "edu_more_magistr" -> { tempEduData.computeIfAbsent(chatId, k -> new HashMap<>()).put("degree", "Magistr"); sendText(chatId, "Magistr universitetinin adını daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_MASTER_UNIVERSITY); }
            case "edu_more_kollec"  -> { tempEduData.computeIfAbsent(chatId, k -> new HashMap<>()).put("degree", "Kollec"); sendText(chatId, "Kollec adını daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_COLLEGE_UNIVERSITY); }
            case "edu_more_yoxdur"  -> { sendText(chatId, "Bacarıqlarınızı daxil edin:\n\nVergüllə ayırın:\n(məs: Java, Spring Boot, SQL)"); userService.updateState(user, UserState.COLLECTING_SKILLS); }
        }
    }

    private void handleTemplateSelect(User user, CvTemplate template) {
        CvData cv = CvData.builder().user(user).template(template).build();
        cvDataRepository.save(cv);
        sendText(user.getChatId(), "✅ Şablon seçildi!\n\nAdınızı və soyadınızı daxil edin:\n(məs: Jale Musayeva)");
    ...
    private void handleEmail(User user, String text) {
        CvData cv = getLatestCvData(user); cv.setEmail(text); cvDataRepository.save(cv);
    ...
    private void handleAddress(User user, String text) {
        CvData cv = getLatestCvData(user); cv.setAddress(text); cvDataRepository.save(cv);
        CvTemplate t = cv.getTemplate();
    ...
    private void handlePhoto(User user, Message message) {
        ...
        try {
            ...
            org.telegram.telegrambots.meta.api.objects.File tgFile = getBot().execute(new GetFile(largest.getFileId()));
            String fileUrl = "https://api.telegram.org/file/bot" + getBot().getBotToken() + "/" + tgFile.getFilePath();
            byte[] photoBytes;
            try (InputStream in = new URL(fileUrl).openStream()) { photoBytes = in.readAllBytes(); }
            CvData cv = getLatestCvData(user); cv.setPhoto(photoBytes); cvDataRepository.save(cv);
            askAbout(user);
        } catch (Exception e) {
    ...
    private void handleAbout(User user, String text) {
        CvData cv = getLatestCvData(user); cv.setAbout(text); cvDataRepository.save(cv);
    ...
    private void handleExpDuties(User user, String text) {
        Map<String, String> exp = tempExpData.get(user.getChatId());
        exp.put("duties", text);
        CvData cv = getLatestCvData(user);
        String existing = cv.getExperience() != null ? cv.getExperience() : "";
        String newExp = "**" + exp.get("company") + " — " + exp.get("position") + "**\n" +
                "**" + exp.get("startDate") + " — " + exp.get("endDate") + "**\n" +
                exp.get("duties");
        cv.setExperience(existing.isEmpty() ? newExp : existing + "\n\n" + newExp);
        cvDataRepository.save(cv);
        tempExpData.put(user.getChatId(), new HashMap<>());
    ...
    private void handleEduBachelorUniversity(User user, String text) { tempEduData.get(user.getChatId()).put("bachUniversity", text); sendText(user.getChatId(), "Bakalavr ixtisasınızı daxil edin:\n(məs: Kompüter Mühəndisliyi)"); userService.updateState(user, UserState.COLLECTING_EDU_BACHELOR_FACULTY); }
    private void handleEduBachelorFaculty(User user, String text) { tempEduData.get(user.getChatId()).put("bachFaculty", text); sendText(user.getChatId(), "Bakalavr başlama ilini daxil edin:\n(məs: 2015)"); userService.updateState(user, UserState.COLLECTING_EDU_BACHELOR_START); }
    private void handleEduBachelorStart(User user, String text) { tempEduData.get(user.getChatId()).put("bachStart", text); sendText(user.getChatId(), "Bakalavr bitmə ilini daxil edin:\n(məs: 2019)"); userService.updateState(user, UserState.COLLECTING_EDU_BACHELOR_END); }
    private void handleEduBachelorEnd(User user, String text) {
        Map<String, String> edu = tempEduData.get(user.getChatId()); edu.put("bachEnd", text);
        CvData cv = getLatestCvData(user);
        String existing = cv.getEducation() != null ? cv.getEducation() : "";
        String entry = "**Bakalavr**\n" + edu.get("bachUniversity") + "\n__" + edu.get("bachFaculty") + "__\n**" + edu.get("bachStart") + " — " + edu.get("bachEnd") + "**";
        cv.setEducation(existing.isEmpty() ? entry : existing + "\n\n" + entry);
        cvDataRepository.save(cv);
        if ("Magistr".equals(edu.get("nextDegree"))) { edu.remove("nextDegree"); sendText(user.getChatId(), "Magistr universitetinin adını daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_MASTER_UNIVERSITY); }
        else { askMoreEdu(user); }
    }
    private void handleEduMasterUniversity(User user, String text) { tempEduData.get(user.getChatId()).put("mastUniversity", text); sendText(user.getChatId(), "Magistr ixtisasınızı daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_MASTER_FACULTY); }
    private void handleEduMasterFaculty(User user, String text) { tempEduData.get(user.getChatId()).put("mastFaculty", text); sendText(user.getChatId(), "Magistr başlama ilini daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_MASTER_START); }
    private void handleEduMasterStart(User user, String text) { tempEduData.get(user.getChatId()).put("mastStart", text); sendText(user.getChatId(), "Magistr bitmə ilini daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_MASTER_END); }
    private void handleEduMasterEnd(User user, String text) {
        Map<String, String> edu = tempEduData.get(user.getChatId()); edu.put("mastEnd", text);
        CvData cv = getLatestCvData(user);
        String existing = cv.getEducation() != null ? cv.getEducation() : "";
        String entry = "**Magistr**\n" + edu.get("mastUniversity") + "\n__" + edu.get("mastFaculty") + "__\n**" + edu.get("mastStart") + " — " + edu.get("mastEnd") + "**";
        cv.setEducation(existing.isEmpty() ? entry : existing + "\n\n" + entry);
        cvDataRepository.save(cv); askMoreEdu(user);
    }
    private void handleEduCollegeUniversity(User user, String text) { tempEduData.get(user.getChatId()).put("collUniversity", text); sendText(user.getChatId(), "Kollec ixtisasınızı daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_COLLEGE_FACULTY); }
    private void handleEduCollegeFaculty(User user, String text) { tempEduData.get(user.getChatId()).put("collFaculty", text); sendText(user.getChatId(), "Kollec başlama ilini daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_COLLEGE_START); }
    private void handleEduCollegeStart(User user, String text) { tempEduData.get(user.getChatId()).put("collStart", text); sendText(user.getChatId(), "Kollec bitmə ilini daxil edin:"); userService.updateState(user, UserState.COLLECTING_EDU_COLLEGE_END); }
    private void handleEduCollegeEnd(User user, String text) {
        Map<String, String> edu = tempEduData.get(user.getChatId()); edu.put("collEnd", text);
        CvData cv = getLatestCvData(user);
        String existing = cv.getEducation() != null ? cv.getEducation() : "";
    ...
    private void askMoreEdu(User user) { tempEduData.put(user.getChatId(), new HashMap<>()); userService.updateState(user, UserState.COLLECTING_EDU_MORE); sendEduMoreInline(user.getChatId()); }

    private void handleSkills(User user, String text) {
        CvData cv = getLatestCvData(user); cv.setSkills(text); cvDataRepository.save(cv);
    ...
    private void handleLanguages(User user, String text) {
        CvData cv = getLatestCvData(user); cv.setLanguages(text); cvDataRepository.save(cv);
        if (cv.isItProfession()) {
    ...
    private void handleGithub(User user, String text) {
        CvData cv = getLatestCvData(user); if (!"yoxdur".equalsIgnoreCase(text.trim())) cv.setGithubLink(text); cvDataRepository.save(cv);
    ...
    private void handleLinkedin(User user, String text) {
        CvData cv = getLatestCvData(user); if (!"yoxdur".equalsIgnoreCase(text.trim())) cv.setLinkedinLink(text); cvDataRepository.save(cv);
        sendText(user.getChatId(), "Layihələriniz varmı?\n\nHər layihəni belə yazın:\nLayihə adı — texnologiyalar — izahat\n\nYoxdursa — \"yoxdur\" yazın");
        userService.updateState(user, UserState.COLLECTING_PROJECTS);
    }
    private void handleProjects(User user, String text) {
        CvData cv = getLatestCvData(user); if (!"yoxdur".equalsIgnoreCase(text.trim())) cv.setProjects(text); cvDataRepository.save(cv);
    ...
    private void handleCertifications(User user, String text) {
        CvData cv = getLatestCvData(user); if (!"yoxdur".equalsIgnoreCase(text.trim())) cv.setCertifications(text); cvDataRepository.save(cv);
        generateAndSendCv(user);
    }
    private void handleComputerSkills(User user, String text) {
        CvData cv = getLatestCvData(user); cv.setComputerSkills(text); cvDataRepository.save(cv);
        sendText(user.getChatId(), "Şəxsi bacarıqlarınızı daxil edin:\n\n(məs: Komanda ilə işləmək, Analitik düşüncə)");
        userService.updateState(user, UserState.COLLECTING_PERSONAL_SKILLS);
    }
    private void handlePersonalSkills(User user, String text) {
        CvData cv = getLatestCvData(user); cv.setPersonalSkills(text); cvDataRepository.save(cv);
        sendText(user.getChatId(), "Sertifikatlarınız varmı?\n\nVarsa belə yazın:\nSertifikat adı — qurum — il\n\nYoxdursa — \"yoxdur\" yazın");
    ...
    private void sendExpMoreInline(Long chatId) {
        InlineKeyboardButton beli = btn("✅ Bəli", "exp_yes");
        InlineKeyboardButton xeyr = btn("❌ Xeyr", "exp_no");
        try { getBot().execute(SendMessage.builder().chatId(chatId.toString()).text("Başqa iş yeriniz varmı?").replyMarkup(new InlineKeyboardMarkup(List.of(List.of(beli, xeyr)))).build()); }
        catch (TelegramApiException e) { log.error("sendExpMoreInline xətası: {}", e.getMessage()); }
    }

    private void sendEduDegreeInline(Long chatId, String text) {
        try { getBot().execute(SendMessage.builder().chatId(chatId.toString()).text(text)
                .replyMarkup(new InlineKeyboardMarkup(List.of(
                        List.of(btn("🎓 Kollec", "edu_kollec"), btn("🎓 Bakalavr", "edu_bakalavr")),
                        List.of(btn("🎓 Magistr", "edu_magistr"), btn("❌ Yoxdur", "edu_yoxdur"))))).build()); }
        catch (TelegramApiException e) { log.error("sendEduDegreeInline xətası: {}", e.getMessage()); }
    }

    private void sendEduMoreInline(Long chatId) {
        try { getBot().execute(SendMessage.builder().chatId(chatId.toString()).text("Başqa təhsiliniz varmı?")
                .replyMarkup(new InlineKeyboardMarkup(List.of(
                        List.of(btn("🎓 Kollec", "edu_more_kollec"), btn("🎓 Bakalavr", "edu_more_bakalavr")),
                        List.of(btn("🎓 Magistr", "edu_more_magistr"), btn("❌ Yoxdur", "edu_more_yoxdur"))))).build()); }
        catch (TelegramApiException e) { log.error("sendEduMoreInline xətası: {}", e.getMessage()); }
    }

    private InlineKeyboardButton btn(String text, String callback) {
        InlineKeyboardButton b = new InlineKeyboardButton();
        b.setText(text); b.setCallbackData(callback); return b;
    }
    private boolean isItProfession(String profession) {
        String lower = profession.toLowerCase();
        for (String k : new String[]{"developer","programmer","engineer","frontend","backend",
                "fullstack","devops","data","software","mobile","web","qa","tester",
                "analyst","architect","proqramçı","tərtibatçı","it ","cyber","network","database","cloud"})
    ...
    public void sendText(Long chatId, String text) {
        try { getBot().execute(SendMessage.builder().chatId(chatId.toString()).text(text).build()); }
        catch (TelegramApiException e) { log.error("Mesaj xətası: {}", e.getMessage()); }
    }

    public void sendText(Long chatId, String text, org.telegram.telegrambots.meta.api.objects.replykeyboard.ReplyKeyboard keyboard) {
        try { getBot().execute(SendMessage.builder().chatId(chatId.toString()).text(text).replyMarkup(keyboard).build()); }
        catch (TelegramApiException e) { log.error("Mesaj xətası: {}", e.getMessage()); }
    }

    private void sendDocument(Long chatId, File file, String fileName) {
        try { getBot().execute(SendDocument.builder().chatId(chatId.toString()).document(new InputFile(file, fileName)).build()); }
        catch (TelegramApiException e) { log.error("Fayl xətası: {}", e.getMessage()); }
    }
}